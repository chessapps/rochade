...
```
internet ──443──> proxy (caddy, ~/stacks/proxy) ──proxy network──> rochade-web:8080 ──> api:8000 ──> postgres
                  rochade.example.com                              rochade-web:8081 ──> zitadel:8080, zitadel-login:3000
                  auth.example.com                                 └── other stacks' web containers
```

Only `web` joins the `proxy` docker network. `api`, `postgres` and Zitadel
stay on the rochade project's own network, unreachable from other stacks.
...
Two `A` records pointing at the box, `rochade.<your-domain>` for the app and
...
```powershell
Copy-Item .env.example rochade.prod.env             # gitignored
...
- `POSTGRES_PASSWORD`, `ZITADEL_MASTERKEY` (exactly 32 characters): secrets,
  generate them.
- `ROCHADE_PUBLIC_URL=https://rochade.<your-domain>`.
...
- Leave `ROCHADE_DEV_AUTH_ENABLED` unset or `false`.
...
the Postgres password later, run `ALTER USER rochade PASSWORD '...'` (and the
...
```sh
curl -I https://rochade.<your-domain>/health        # 200, valid certificate
curl https://rochade.<your-domain>/api/auth/config  # names https://auth.<your-domain> and a client id
curl https://auth.<your-domain>/.well-known/openid-configuration
```

Open `https://rochade.<your-domain>/`: the landing page names the site, tells a
player to scan the QR and links an arbiter to `/admin/`. With
`ROCHADE_DEVICE_JOIN_ENABLED` unset it shows no join code field, so nothing on
...
```sh
ZITADEL_ADMIN_EMAIL=... ZITADEL_ADMIN_PASSWORD=... node scripts/login_flow.mjs https://rochade.<your-domain>
```
...
one: rochade's `web` container joins its network (`PROXY_NETWORK` in the env
...
```
0 3 * * * mkdir -p ~/backups/rochade && docker exec rochade-postgres-1 pg_dump -U rochade -Fc rochade > ~/backups/rochade/$(date +\%F).dump && find ~/backups/rochade -name '*.dump' -mtime +14 -delete
```
...
and the password an API key of the IAM application `rochade-zitadel`
...
later ones, only refreshes its redirect URIs. If `ROCHADE_PUBLIC_URL` changes,
redeploying is enough.

`ROCHADE_DEV_AUTH_ENABLED=true` beside Zitadel would let any bare bearer act as
