...
personal access token Zitadel wrote for its bootstrap machine user:
    ...
    python -m rochade.platform.auth.zitadel_setup
...
result lands in a JSON file the API reads at start (ROCHADE_OIDC_CONFIG_FILE).
...
Environment:
    ...
    ROCHADE_PUBLIC_URL   the app's origin, for the redirect URIs
    ROCHADE_OIDC_CONFIG_FILE  where to write the result
...
def main() -> int:
    ...
    public_url = os.environ["ROCHADE_PUBLIC_URL"].rstrip("/")
    out = pathlib.Path(os.environ["ROCHADE_OIDC_CONFIG_FILE"])
